...
public class CircleProgressBar extends ImageView implements MaterialHeadListener {
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        mProgressDrawable.setSizeParameters((double) mDiameter, (double)mDiameter,
                mInnerRadius <= 0 ? (double)(mDiameter - mProgressStokeWidth * 2) / 4 : (double)mInnerRadius,
                (double)mProgressStokeWidth,
                mArrowWidth < 0 ? (float)mProgressStokeWidth * 4 : (float)mArrowWidth,
                mArrowHeight < 0 ? (float)mProgressStokeWidth * 2 : (float)mArrowHeight);
        if (isShowArrow()) {
...
//        mProgressDrawable.setStartEndTrim(0, (float) 0.75);
    ...
    private class OvalShadow extends OvalShape {
        ...
        public OvalShadow(int shadowRadius, int circleDiameter) {
            ...
            mRadialGradient = new RadialGradient(mCircleDiameter / 2.0f, mCircleDiameter / 2.0f,
                    (float)mShadowRadius, new int[]{
        ...
        public void draw(Canvas canvas, Paint paint) {
            ...
            canvas.drawCircle(viewWidth / 2.0f, viewHeight / 2.0f, (mCircleDiameter / 2.0f + mShadowRadius),
                    mShadowPaint);
            canvas.drawCircle(viewWidth / 2.0f, viewHeight / 2.0f, (mCircleDiameter / 2.0f), paint);
        }
    }
}
